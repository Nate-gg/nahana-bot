	...
	execute(message, args) {
		...
		if (birthdays.find(obj => obj.user === member.id)) {
			return message.channel.send(
				rsp('We already have your birthday', errorImg)
			)
		}
		...
		if (!isValid) {
			return message.channel.send(
				rsp('Please Enter A Valid Date', errorImg)
			)
		}
		...
		message.channel.send(
			rsp(`Thanks ${user}. Your Bday has been added!`, okImg)
		)
	},
}

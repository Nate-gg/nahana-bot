...
module.exports = {
	...
	execute(message, args) {
		...
		const BRBembed = new Discord.MessageEmbed()
			...
			.setTitle('One Sec')
			.setImage(
				'https://cdn.discordapp.com/emojis/700191725846790276.gif?v=1'
			)
		...
		const getPic = () => {
			if (int === max) {
				...
				return message.channel.send(
					rsp(
						"Sorry ... I couldn't find any images to show",
						errorImg
					)
				)
			}
			Reddit.random(sub)
				.then(res => {
					...
					const isImage = imageExtensions.find(
				...
				.catch(() => {
					brb.delete()
					return message.channel.send(
						rsp(`r/${sub} doesn't exist`, errorImg)
					)
				})
		}
